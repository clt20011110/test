    ...
    case fullLine wid grid of
        ...
        Just lineY ->
            let
                ...
                posc =
                    List.map (\a -> a.pos) clearedGrid

                posa =
                    List.map (\a -> a.pos) above

                linec =
                    case List.unzip posc |> Tuple.second |> List.maximum of
                        Nothing ->
                            0
                ...
                linea =
                    case List.unzip posa |> Tuple.second |> List.maximum of
                        Nothing ->
                            0
                ...
                droppedAbove =
                    List.map (\c -> { c | pos = ( Tuple.first c.pos, Tuple.second c.pos + line ) }) above

                ( newGrid, lines ) =
                    clearLines wid droppedAbove
            in
            ( newGrid, lines + 1 )
